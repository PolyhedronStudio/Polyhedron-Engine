...
void Weapon_HyperBlaster (edict_t *ent)
{
	...
	case WEAPON_ACTIVATING:
		// just go forward until we're activated
		ent->client->ps.gunframe++;
	...
	case WEAPON_DROPPING:
		// just go forward until we're deactivated
		ent->client->ps.gunframe++;

		const int ANIM = (ent->client->ps.gunframe - PUTAWAY_START) / PUTAWAY_LENGTH;
		const int SUBFRAME = (ent->client->ps.gunframe - PUTAWAY_START) % PUTAWAY_LENGTH;

		if (SUBFRAME >= PUTAWAY_LENGTH - 1)
			ChangeWeapon(ent);
		break;
	case WEAPON_READY: {
		// we can land on a ready frame at any time, and we'll just play the
		// animation until we land on the end of the after frame
		const int ANIM = (ent->client->ps.gunframe - FIRE_START) / FIRE_LENGTH;
		const int SUBFRAME = (ent->client->ps.gunframe - FIRE_START) % FIRE_LENGTH;

		// hold on whatever frame we're on

		if (ent->client->newweapon)
		{
			ent->client->weaponstate = WEAPON_DROPPING;
			ent->client->ps.gunframe = PUTAWAY_START + (((ANIM + 1) % NUM_FIRE_ANIMS) * PUTAWAY_LENGTH);
	...
	case WEAPON_FIRING: {
		...
		{
			// check for re-fire
			if (((ent->client->latched_buttons | ent->client->buttons) & BUTTONS_ATTACK) && !ent->client->newweapon)
			{
				if ((!ent->client->ammo_index) || 
					( ent->client->pers.inventory[ent->client->ammo_index] >= ent->client->pers.weapon->quantity))
				{
			...
			if (ent->client->weaponstate == WEAPON_FIRING && SUBFRAME == FIRE_LENGTH - 1)
			{
