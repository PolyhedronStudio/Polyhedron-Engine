...
{
	if (!st.grFogOnOff || !*st.grFogOnOff)
	{
		gi.dprintf("target_grfog with no grFogOnOff string at %s\n", vtos(self->s.origin));
		G_FreeEdict(self);
		return;
	}

	self->pathtarget = gi.TagMalloc(64 + 1, TAG_LEVEL);
	strcpy(self->pathtarget, st.fog);
	...
	self->grFogDenistyRoot = gi.TagMalloc(64 + 1, TAG_LEVEL);
	strcpy(self->grFogDenistyRoot, st.grFogDenistyRoot);
...
void SP_trigger_grfog(edict_t *self)
{
	...
	self->delay = (st.grFogDelay * 1000.0f);
