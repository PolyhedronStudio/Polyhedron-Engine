...
player_update(server_frame_t *oldframe, server_frame_t *frame, int framediv)
{
    ...
    if (fabsf((float)(ops->pmove.origin[0] - ps->pmove.origin[0])) > 256 ||
        fabsf((float)(ops->pmove.origin[1] - ps->pmove.origin[1])) > 256 ||
        fabsf((float)(ops->pmove.origin[2] - ps->pmove.origin[2])) > 256) {
        goto dup;
    }
...
{
    // CL_CalculateViewValues(); // N&C: Moved to V_RenderView so CG Module can use these too.
   // CL_FinishViewValues();
