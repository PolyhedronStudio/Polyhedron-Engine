...
// PF_RunPlayerPhysics
// 
// Taken and modified from FTEQW. Emulates player movement code on regular entities.
// Useful for implementing AI.
//===============
//
static void PF_RunPlayerPhysics(edict_t *ent) {
    unsigned int i, n;
    extern byte *playertouch;
    ...
    if (!ent)
    {
        Com_Printf("Tried to execute RunPlayerPhysics on an invalid entity\n");
        return;
    }

    // 
    //if (ent->num >= 1 && ent->entnum <= sv.allocated_client_slots)
...
void SV_InitGameProgs(void)
{
    ...
    import.RunPlayerPhysics = PF_RunPlayerPhysics;
