...
static void CL_AreaEntities_r(areanode_t *node) {
    ...
    if (areaMaxs[node->axis] > node->dist)
        CL_AreaEntities_r(node->children[0]);
    if (areaMins[node->axis] < node->dist)
        CL_AreaEntities_r(node->children[1]);
}
...
int32_t CL_PointContents(const vec3_t &point)
{
    ...
    for (int32_t i = 0; i < numberOfAreaEntities; i++) {
        ...
        contents |= CM_TransformedPointContents(point, CL_HullForEntity(hit), hit->state.origin, hit->state.angles);
    }
...
static void CL_ClipMoveToEntities(const vec3_t &start, const vec3_t &mins, const vec3_t &maxs, const vec3_t &end, Entity *passedict, int32_t contentMask, TraceResult *tr) {
    ...
    for (int32_t i = 0; i < numberOfAreaEntities; i++) {
        ...
        // Might intersect, so do an exact clip
		TraceResult trace = CM_TransformedBoxTrace(start, end, mins, maxs, CL_HullForEntity(touchEntity), contentMask, touchEntity->state.origin, touchEntity->state.angles);
