...
const float plane_distance( const CollisionPlane &p, const vec3_t &point, const float extraDistance = 0.f ) {
	const vec3_t planeOrigin = vec3_scale( p.normal, p.dist + extraDistance );
...
const float sphere_plane_project( const vec3_t &spherePos, const CollisionPlane & plane ) {
  //      return fabs( spherePos.x * plane.normal.x 
		//			+ spherePos.y * plane.normal.y 
		//			+ spherePos.z * plane.normal.z 
		//			+ plane.dist 
		//);
...
const float sphere_plane_collision_distance( const vec3_t &point, const sphere_t &sphere, const CollisionPlane &plane ) {
		return fabs(  
			sphere_plane_project( point, plane ) 
			/ sqrtf( vec3_dot( plane.normal, plane.normal ) )// plane.normal.x * plane.normal.x + plane.normal.y * plane.normal.y + plane.normal.z * plane.normal.z ) 
		);
}
/**
