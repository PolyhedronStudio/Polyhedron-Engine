...
*	@file
*
*	MonsterSlideBoxDummy implementation.
*
***/
//! Main Headers.
...
#include "Game/Shared/Physics/SlideBox.h"
...
#include "Game/Server/Entities/Base/SVGBaseRootMotionAnimator.h"
...
// Monster SlideBox Dummy
#include "MonsterSlideBoxDummy.h"


/**
*	@brief	Constructor
**/
MonsterSlideBoxDummy::MonsterSlideBoxDummy( PODEntity *podEntity ) : Base( podEntity ) { 
}
...
**/
void MonsterSlideBoxDummy::Precache() {
...
**/
void MonsterSlideBoxDummy::Spawn() {
    // Always call parent class method.
    Base::Spawn();
    ...
    SetDieCallback( &MonsterSlideBoxDummy::DieCallback_FallDead );

	// Make it so that the player can toggle '+use' this monster.
	SetUseEntityFlags( UseEntityFlags::Toggle );
	SetUseCallback( &MonsterSlideBoxDummy::UseCallback_EngageGoal );

	// Setup thinking.
    SetThinkCallback( &MonsterSlideBoxDummy::ThinkCallback_General );
...
void MonsterSlideBoxDummy::PostSpawn() {
    ...
    SetThinkCallback(&MonsterSlideBoxDummy::Callback_DetermineSpawnAnimation);
    SetNextThinkTime(level.time + FRAMETIME_S);
}
...
**/
void MonsterSlideBoxDummy::Think() {
	// Always call parent class method.
    Base::Think();
...
**/
void MonsterSlideBoxDummy::SpawnKey(const std::string& key, const std::string& value) {
	...
	if ( key == "monstergoaltarget" ) {
		...
		gi.DPrintf("MonsterSlideBoxDummy(GoalEntity: %s)\n", value.c_str());
	} else {
...
**/
void MonsterSlideBoxDummy::UseCallback_EngageGoal(IServerGameEntity *other, IServerGameEntity* activator) {
...
// 
void MonsterSlideBoxDummy::Callback_DetermineSpawnAnimation(void) { 
...
**/
void MonsterSlideBoxDummy::ThinkCallback_General(void) {
...
**/
void MonsterSlideBoxDummy::DieCallback_FallDead(IServerGameEntity* inflictor, IServerGameEntity* attacker, int damage, const vec3_t& point) {
	// Get Gameworld.
...
//    SVG_Sound( this, SoundChannel::Body, SVG_PrecacheSound("misc/gibdeath1.wav"), 1, Attenuation::Normal, 0 );
 ...
 //   gameWorld->ThrowGib( this, "models/objects/gibs/sm_meat/tris.md2", 12, damage, GibType::Organic );
    ...
    SetNextThinkTime(level.time + FRAMETIME_S);
	SetDieCallback( &MonsterSlideBoxDummy::Callback_MorphToClientGibs );
}

/**
*	@brief	Prepares the entity for removement after spawning various client gib events.
**/
void MonsterSlideBoxDummy::Callback_MorphToClientGibs(IServerGameEntity* inflictor, IServerGameEntity* attacker, int damage, const vec3_t& point) {
    // Attacker becomes this entity its "activator".
    SetActivator(attacker);
	...
	SetNextThinkTime(level.time + FRAMETIME_S);
	SetThinkCallback(&MonsterSlideBoxDummy::SVGBaseEntityThinkFree);
}
...
**/
float MonsterSlideBoxDummy::TurnToIdealYawAngle() {
...
**/
const int32_t MonsterSlideBoxDummy::PerformRootMotionMove() {
	// Get GameWorld.
...
**/
void MonsterSlideBoxDummy::DebugPrint(const int32_t entityNumber, const int32_t resultMask, const int32_t previousResultMask, const int32_t moveFlags, const int32_t moveFlagTime) {
...
**/
const int32_t MonsterSlideBoxDummy::NavigateToOrigin( const vec3_t &navigationOrigin ) {
	/**
