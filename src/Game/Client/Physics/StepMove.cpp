...
realcheck:
    ...
    trace = CLG_Trace(start, vec3_zero(), vec3_zero(), stop, ent, BrushContentsMask::MonsterSolid);
...
void CLG_StepMove_CheckGround(CLGBaseEntity* ent)
{
    ...
    trace = CLG_Trace(ent->GetOrigin(), ent->GetMins(), ent->GetMaxs(), point, ent, BrushContentsMask::MonsterSolid);
...
qboolean CLG_MoveStep(CLGBaseEntity* ent, vec3_t move, qboolean relink)
{
    ...
    if (ent->GetFlags() & (EntityFlags::Swim | EntityFlags::Fly)) {
        ...
        for (i = 0; i < 2; i++) {
            ...
            trace = CLG_Trace(ent->GetOrigin(), ent->GetMins(), ent->GetMaxs(), newOrigin, ent, BrushContentsMask::MonsterSolid);
    ...
    trace = CLG_Trace(newOrigin, ent->GetMins(), ent->GetMaxs(), end, ent, BrushContentsMask::MonsterSolid);
    ...
    if (!CLG_StepMove_CheckBottom(ent)) {
        if (ent->GetFlags() & EntityFlags::PartiallyOnGround) {
