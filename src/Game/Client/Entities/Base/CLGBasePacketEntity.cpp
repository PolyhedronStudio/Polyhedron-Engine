...
void CLGBasePacketEntity::PrepareRefreshEntity(const int32_t refreshEntityID, EntityState *currentState, EntityState *previousState, float lerpFraction) {
 ...
 //   r_entity_t   refreshEntity = {}; // Ensure it is clear aka set to 0.
...
//    EntityState* currentState = nullptr;
 ...
 //   PODEntity* podEntity = nullptr;
    ...
    // Entity specific _effects. (Such as whether to rotate or not.)
    uint32_t _effects = 0;
    // Entity render _effects. (Shells and the like.)
    uint32_t _renderEffects = 0;
    ...
    int32_t autoAnimation = BASE_FRAMERATE * cl->time / BASE_FRAMETIME_1000;
        ...
        //currentState = &cl->entityStates[entityIndex];
        // Fetch the actual entity to process based on the entity's state index number.
        //podEntity = &cs->entities[currentState->number];
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////
        //Com_DPrint("entityIndex=%i, currentState->number=%i, podEntity->id=%i, pointernr=%i\n", entityIndex, currentState->number, podEntity->id, pointerNumber);
...
// 
        ...
        // Effects.
        // 
        // Fetch the _effects of current entity.
        _effects = currentState->effects;
        // Fetch the render _effects of current entity.
        _renderEffects = currentState->renderEffects;

        //
        // Frame Animation Effects.
        //
        if (_effects & EntityEffectType::AnimCycleFrames01hz2)
            refreshEntity.frame = autoAnimation & 1;
        else if (_effects & EntityEffectType::AnimCycleFrames23hz2)
            refreshEntity.frame = 2 + (autoAnimation & 1);
        else if (_effects & EntityEffectType::AnimCycleAll2hz)
            refreshEntity.frame = autoAnimation;
        else if (_effects & EntityEffectType::AnimCycleAll30hz)
        ...
        if (cl_noglow->integer)
            _renderEffects &= ~RenderEffects::Glow;

        //
        // Setup refreshEntity origin.
        //
        if (_renderEffects & RenderEffects::FrameLerp) {
        ...
        } else if (_renderEffects & RenderEffects::Beam) {
	    ...
	    if (_renderEffects & RenderEffects::DebugBoundingBox) {
	        CLG_DrawDebugBoundingBox(podEntity->lerpOrigin, podEntity->mins, podEntity->maxs);
	    }

        // tweak the color of beams
        if (_renderEffects & RenderEffects::Beam) {
            ...
            if (currentState->modelIndex == 255) {
                ...
                if (_renderEffects & RenderEffects::UseDisguise) {
                ...
                if (refreshEntity.model == cl_mod_laser || refreshEntity.model == cl_mod_dmspot)
                    _renderEffects |= RF_NOSHADOW;
            }
        }

        // Only used for black hole model right now, FIXME: do better
        if ((_renderEffects & RenderEffects::Translucent) && !(_renderEffects & RenderEffects::Beam)) {
            refreshEntity.alpha = 0.70;
        }

        // Render _effects (fullbright, translucent, etc)
        if ((_effects & EntityEffectType::ColorShell)) {
            refreshEntity.flags = 0;  // Render _effects go on color shell entity
        } else {
            refreshEntity.flags = _renderEffects;
        }

        //
        // Angles.
        //
        if (_effects & EntityEffectType::Rotate) {
        ...
        // In rtx mode, the base entity has the _renderEffects for shells
        if ((_effects & EntityEffectType::ColorShell) && vid_rtx->integer) {
//            _renderEffects = ApplyRenderEffects(_renderEffects);
            refreshEntity.flags |= _renderEffects;
        }
        ...
        if ((_effects & EntityEffectType::ColorShell) && !vid_rtx->integer) {
//            _renderEffects = ApplyRenderEffects(_renderEffects);
            refreshEntity.flags = _renderEffects | baseEntityFlags | RenderEffects::Translucent;
            refreshEntity.alpha = 0.30;
            clge->view->AddRenderEntity(refreshEntity);
        }
        ...
        // 
        // Add automatic particle trail _effects where desired.
        if (_effects & ~EntityEffectType::Rotate) {
            if (_effects & EntityEffectType::Gib) {
                ParticleEffects::DiminishingTrail(podEntity->lerpOrigin, refreshEntity.origin, podEntity, _effects);
            } else if (_effects & EntityEffectType::Torch) {
 ...
 //   const uint32_t _effects = currentState->effects;
 //   // Entity Render Effects set by game logic.
 //   int32_t _renderEffects = currentState->renderEffects;
	//// Bonus items rotate at a fixed rate
	//float autoRotate = AngleMod(cl->time * BASE_1_FRAMETIME);
	//// Brush models can auto animate their frames
	//int32_t autoAnimation = BASE_FRAMERATE * cl->time / BASE_FRAMETIME_1000;
