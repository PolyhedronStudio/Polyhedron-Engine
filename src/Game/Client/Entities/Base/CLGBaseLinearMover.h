...
**/
struct LinearPushMoveInfo {
	/**
	*	Old stuff, filter out later.
	**/
	...
	**/
	...
	vec3_t linearMovementBegin = vec3_zero();
	vec3_t linearMovementEnd = vec3_zero();
	vec3_t linearMovementVelocity = vec3_zero();
	GameTime linearMovementDuration = GameTime::zero();
	GameTime linearMovementTimeStamp = GameTime::zero();

    // Callback function to use when the move has ended.
    LinearPushMoveEndFunction* OnEndFunction = nullptr;
};
...
*			case of a MoveType::Push it'll start pushing the blocking object around. All CLGBaseMover
*			brush entities themselves do not collide interact with each other.
**/
class CLGBaseLinearMover : public CLGBaseTrigger {
public:
    // Constructor/Deconstructor.
    CLGBaseLinearMover(PODEntity *svEntity);
    virtual ~CLGBaseLinearMover() = default;

	// Inherit from CLGBaseTrigger.
    DefineAbstractClass( CLGBaseLinearMover, CLGBaseTrigger );
	...
	**/
    ...
    // Get/Set.
    //
    inline LinearPushMoveInfo* GetPushMoveInfo() {
        return &moveInfo;
    }
...
protected:
    ...
    void        BrushAngleMoveCalc( LinearPushMoveEndFunction* function );
