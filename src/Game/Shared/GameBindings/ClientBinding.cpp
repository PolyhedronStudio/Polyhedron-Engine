...
GameEntityVector SG_BoxEntities( const vec3_t& mins, const vec3_t& maxs, int32_t listCount, const int32_t areaType ) {
    // Boxed entities set by clgi.BoxEntities.
    PODEntity* boxedEntities[MAX_CLIENT_POD_ENTITIES];
    ...
    // Box the entities.
    const int32_t numEntities = clgi.BoxEntities( mins, maxs, boxedEntities, listCount, areaType );
	...
	for (int32_t i = 0; i < numEntities; i++) {
        if ( gameEntities[ boxedEntities[i]->clientEntityNumber ] != nullptr ) {
            boxedGameEntities.push_back( gameEntities[ boxedEntities[i]->clientEntityNumber ] );
        }
    }
