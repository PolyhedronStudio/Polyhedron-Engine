...
int32_t Huff_getBit(byte* fin, int32_t* offset) {
	int32_t t;
	bloc = *offset;
...
static int32_t get_bit(byte* fin) {
	int32_t t;
	t = (fin[(bloc >> 3)] >> (bloc & 7)) & 0x1;
	bloc++;
	return t;
...
void Huff_Decompress(msg_t* mbuf, int32_t offset) {
	int32_t			ch, cch, i, j, size;
	byte		seq[65536];
	byte* buffer;
	huff_t		huff;
...
void Huff_Compress(msg_t* mbuf, int32_t offset) {
	int32_t i, ch, size;
	byte seq[65536];
	byte* buffer;
	huff_t		huff;
