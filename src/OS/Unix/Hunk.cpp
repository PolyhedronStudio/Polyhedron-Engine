...
void Hunk_Begin(memhunk_t *hunk, size_t maximumSize)
{
    ...
    if (maximumSize > SIZE_MAX - 4095)
        Com_Error(ErrorType::Fatal, "%s: size > SIZE_MAX", __func__);
    ...
    if (buf == NULL || buf == (void *)-1)
        Com_Error(ErrorType::Fatal, "%s: unable to reserve %" PRIz " bytes: %s",
...
void *Hunk_Alloc(memhunk_t *hunk, size_t size)
{
    ...
    if (hunk->currentSize > hunk->maximumSize)
        Com_Error(ErrorType::Fatal, "%s: currentSize > maximumSize", __func__);

    if (size > hunk->maximumSize - hunk->currentSize)
        Com_Error(ErrorType::Fatal, "%s: couldn't allocate %" PRIz " bytes", __func__, size);
...
#endif
        if (buf != hunk->base)
            Com_Error(ErrorType::Fatal, "%s: could not remap virtual block: %s",
...
void Hunk_Free(memhunk_t *hunk)
{
    if (hunk->base && munmap(hunk->base, hunk->mapped))
        Com_Error(ErrorType::Fatal, "%s: munmap failed: %s",
                  __func__, strerror(errno));
