...
void PutClientInServer(edict_t *ent)
{
	...
	if (ITEM_INDEX(client->pers.weapon) == noweapon_index)
		...
		ent->s.modelindex2 = MAX_MODELS;	// custom gun model
	...
	if (spawn_levelchange && !client->pers.chasetoggle && !client->pers.newweapon)
	{
		...
		i = ((client->pers.weapon->weapmodel & 0xff) << 8L);
	...
	else
	{
		...
		//if (client->pers.weapon->ammo != NULL)
		//{
		//	client->ammo_index = ITEM_INDEX(FindItem(client->pers.weapon->ammo));
		//	gitem_t* ammo = FindItem(client->pers.weapon->ammo);
		//	Add_Ammo(ent, ammo, 25);
		//}
		ChangeWeapon(ent);
	}
