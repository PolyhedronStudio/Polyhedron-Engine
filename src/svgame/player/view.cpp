...
static void SVG_Player_CheckWorldEffects(void)
{
    ...
    if (waterlevel == 3) {
        ...
        if (currentPlayer->GetAirFinishedTime() < level.time) {
            ...
            if (currentPlayer->GetNextDrownTime() < level.time
                ...
                SVG_Damage(currentPlayer, SVG_GetWorldClassEntity(), SVG_GetWorldClassEntity(), vec3_origin, currentPlayer->GetOrigin(), vec3_origin, currentPlayer->GetDamage(), 0, DamageFlags::NoArmorProtection, MeansOfDeath::Water);
            }
        }
    } else {
    ...
    if (waterlevel && (currentPlayer->GetWaterType() & (CONTENTS_LAVA | CONTENTS_SLIME))) {
        if (currentPlayer->GetWaterType() & CONTENTS_LAVA) {
            ...
            SVG_Damage(currentPlayer, SVG_GetWorldClassEntity(), SVG_GetWorldClassEntity(), vec3_origin, currentPlayer->GetOrigin(), vec3_zero(), 3 * waterlevel, 0, 0, MeansOfDeath::Lava);
        }

        if (currentPlayer->GetWaterType() & CONTENTS_SLIME) {
            SVG_Damage(currentPlayer, SVG_GetWorldClassEntity(), SVG_GetWorldClassEntity(), vec3_origin, currentPlayer->GetOrigin(), vec3_zero(), 1 * waterlevel, 0, 0, MeansOfDeath::Slime);
        }
    }
}
