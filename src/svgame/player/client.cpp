...
void PrintPMove(PlayerMove *pm)
{
    ...
    c2 = CheckBlock(&pm->clientMoveCommand, sizeof(pm->clientMoveCommand));
    Com_Printf("sv %3i:%i %i\n", pm->clientMoveCommand.moveInput.impulse, c1, c2);
}
...
==============
*/
void SVG_ClientThink(Entity *serverEntity, ClientMoveCommand *clientMoveCommand)
{
    ...
    if (level.intermission.time) {
        ...
        if (level.time > level.intermission.time + 5.0
            && (clientMoveCommand->moveInput.buttons & BUTTON_ANY))
    ...
    if (client->chaseTarget) {
        ...
        client->respawn.commandViewAngles[0] = clientMoveCommand->moveInput.viewAngles[0];
        client->respawn.commandViewAngles[1] = clientMoveCommand->moveInput.viewAngles[1];
        client->respawn.commandViewAngles[2] = clientMoveCommand->moveInput.viewAngles[2];
    } else {
        ...
        pm.clientMoveCommand = *clientMoveCommand;
        ...
        if (classEntity->GetGroundEntity() && !pm.groundEntityPtr && (pm.clientMoveCommand.moveInput.upMove >= 10) && (pm.waterLevel == 0)) {
    ...
    client->buttons = clientMoveCommand->moveInput.buttons;
    client->latchedButtons |= client->buttons & ~client->oldButtons;

    // save light level the player is standing on for
    // monster sighting AI
    //ent->lightLevel = clientMoveCommand->moveInput.lightLevel;
    ...
    if (client->respawn.isSpectator) {
        if (clientMoveCommand->moveInput.upMove >= 10) {
