...
//	sprintf (infilename, "baseq2/%s", DEADSOLDIER_MODEL);
	Com_sprintf (infilename, sizeof(infilename), "basenac/%s", DEADSOLDIER_MODEL);
	if ( !(infile = fopen (infilename, "rb")) )
	{
		...
		if (!data)
		{
			gi.dprintf("PatchDeadSoldier: Could not find %s in basenac/pak0.pak\n",DEADSOLDIER_MODEL);
			return 0;
		}
	}
	else
