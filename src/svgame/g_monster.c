...
//	Com_sprintf (outfilename, sizeof(outfilename), "%s/%s", gamedir->string, modelname);
	...
	// load original model
	Com_sprintf (infilename, sizeof(infilename), "basenac/%s", modelname);
	...
	if ( !(infile = fopen (infilename, "rb")) )
	{
		...
		// Knightmare- look in all pak files in basenac
		for (i=0; i<10; i++)
		{
			Com_sprintf (pakfile, sizeof(pakfile), "basenac/pak%i.pak", i);
		//	fpak = fopen("basenac/pak0.pak","rb");
			...
			if (!fpak)
			{
				...
				Com_sprintf(pakfile, sizeof(pakfile), "%s/basenac/pak0.pak",cddir->string);
				fpak = fopen(pakfile,"rb");
				if (!fpak)
				{
		...
		if (!data) // if not in basenac pak file, check pakfiles in current gamedir
		{
		...
		if (!data)
		{
			gi.dprintf("PatchMonsterModel: Could not find %s in basenac/pak0.pak\n",modelname);
			return 0;
		}
	}
	else
