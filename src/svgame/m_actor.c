	...
	for (i=game.maxclients+1; i<globals.num_edicts; i++) {
		...
		if (!s_match) {
			...
			for (j=0; j<NUM_ACTOR_SOUNDS; j++) {
				...
				// Search in basenac for external file
				Com_sprintf(filename, sizeof(filename), "%s/basenac/sound/%s%s",basedir->string,path,wavname[j]);
				f = fopen(filename,"r");
				if (f) {
					fclose(f);
					Q_strncpyz(filename, path, sizeof(filename));
				...
				// Search paks in basenac
				Com_sprintf(filename, sizeof(filename), "sound/%s%s",path,wavname[j]);
				if (InPak(basedir->string,"basenac",filename)) {
					Q_strncpyz(filename, path, sizeof(filename));
					Q_strncatz(filename, wavname[j], sizeof(filename));
					e->actor_sound_index[j] = gi.soundindex(filename);
					continue;
				...
				if (strlen(cddir->string))
				{
					...
					Com_sprintf(filename, sizeof(filename), "%s/basenac/sound/%s%s",cddir->string,path,wavname[j]);
					f = fopen(filename,"r");
					if (f) {
						fclose(f);
						Q_strncpyz(filename, path, sizeof(filename));
					...
					// Search paks in basenac
					Com_sprintf(filename, sizeof(filename), "sound/%s%s",path,wavname[j]);
					if (InPak(cddir->string,"basenac",filename)) {
						Q_strncpyz(filename, path, sizeof(filename));
						Q_strncatz(filename, wavname[j], sizeof(filename));
						e->actor_sound_index[j] = gi.soundindex(filename);
						continue;
		...
		for (k=0; k<2; k++) {
			...
			// Search in basenac for external file
			Com_sprintf(path, sizeof(path), "%s/basenac/%s",basedir->string,filename);
			f = fopen(path,"r");
			if (f) {
				fclose(f);
				e->actor_model_index[k] = gi.modelindex(filename);
			...
			if (strlen(cddir->string))
			{
				...
				Com_sprintf(path, sizeof(path), "%s/basenac/%s",cddir->string,filename);
				f = fopen(path,"r");
				if (f) {
					fclose(f);
					e->actor_model_index[k] = gi.modelindex(filename);
